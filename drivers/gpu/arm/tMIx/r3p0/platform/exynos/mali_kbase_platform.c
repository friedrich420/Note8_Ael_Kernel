	...
	if (lotid_info == 0x3A8D3) {    /* core mask code for KC first lot */
		...
		if (core_info) {	/* has dead core more 1-core */
			...
			for (i = 0; i < 8; i++) {
				...
				if (core_stack[i] == 0xb)	core_stack[i] = 3;	/* 0b1011 */
				if (core_stack[i] == 0xd)	core_stack[i] = 1;	/* 0b1101 */
				if (core_stack[i] == 0x9)	core_stack[i] = 1;	/* 0b1001 */
				if (core_stack[i] == 0x5)	core_stack[i] = 1;	/* 0b101  */
				if (!(core_stack[i] == 0x1 || core_stack[i] == 0x3 || core_stack[i] == 0x7 || core_stack[i] == 0xf))	core_stack[i] = 0;
		...
		if (core_info) {        /* has dead core more 1-core */
			...
			for (i = 0; i < 8; i++) {
				...
				if (core_stack[i] == 0x1011)    core_stack[i] = 0x0011;         /* 0b1011 */
				if (core_stack[i] == 0x1101)    core_stack[i] = 0x0001;         /* 0b1101 */
				if (core_stack[i] == 0x1001)    core_stack[i] = 0x0001;         /* 0b1001 */
				if (core_stack[i] == 0x101)             core_stack[i] = 0x0001; /* 0b101 */
				if (!(core_stack[i] == 0x1 || core_stack[i] == 0x11 || core_stack[i] == 0x111 || core_stack[i] == 0x1111))      core_stack[i] = 0;
				if(i==1 || i==5) core_stack[i] = core_stack[i] << 1;
				if(i==2 || i==6) core_stack[i] = core_stack[i] << 2;
				GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "[GPU] after core stack[%d] = 0x%llx\n", i, core_stack[i]);
...
static bool exynos_secure_mode_supported(struct kbase_device *kbdev)
{
	return true;
}
...
struct kbase_protected_ops exynos_secure_ops= {
	...
	.protected_mode_supported = exynos_secure_mode_supported,
};
#endif
